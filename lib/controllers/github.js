...
module.exports = Router()
    ...
    .get('/callback', async (req, res, next) => {
        try {
            ...
            const payload = jwt.sign(JSON.stringify(user), process.env.JWT_SECRET);

            res.cookie(process.env.COOKIE_NAME, payload, { httpOnly: true, maxAge: ONE_DAY_IN_MS }).redirect('/api/v1/github/test')
        } catch(e) {
            next(e);
        }
    });